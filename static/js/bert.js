...
function deleteCommentHandler(e) {
  const commentContainer = e.target.closest("article.comment__container");
  const wrapperNode = commentContainer.parentNode;
...
function postComment(comment) {
  ...
  const predictedLabel = predictSentiment(comment);

  const commentNode = getCommentNode(comment);
  const labelNode = document.createElement("span");
  labelNode.textContent = predictedLabel;
  labelNode.style.color = "white";
  labelNode.style.fontSize = "12px";
  labelNode.style.padding = "5px";

  // Set background color based on sentiment
  switch (predictedLabel) {
    case "Tích Cực":
      labelNode.style.backgroundColor = "green";
      break;
    case "Trung Tính":
      labelNode.style.backgroundColor = "orange";
      break;
    case "Tiêu Cực":
      labelNode.style.backgroundColor = "red";
      break;
    default:
      // Set a default color if the label is not recognized
      labelNode.style.backgroundColor = "orange";
  }

  const wrapperNode = document.createElement("div");
  wrapperNode.style.display = "flex";
  wrapperNode.style.alignItems = "center";
  wrapperNode.appendChild(commentNode);
  wrapperNode.appendChild(labelNode);
  commentsContainerElement.prepend(wrapperNode);
}


function predictSentiment(comment) {
  // Send a POST request to Flask to get the prediction result
  const xhr = new XMLHttpRequest();
  xhr.open("POST", "/bert", false);
